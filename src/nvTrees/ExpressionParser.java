...
public class ExpressionParser {
	...
	private static void init()
	{
		...
		unary_opeators.add("inverse "); //added to test multicharacter operators
		unary_opeators.add("$"); //reduce
		unary_opeators.add("@"); //(un)reduce to smallest grid on the left
		unary_opeators.add("growth "); //growth [g_1,g_2,..g_k,N] computes the sizes of B_1(id), ... B_N(id) in the GROUP generated by g1_,...,g_k 
		unary_opeators.add("semi "); //same as growth, but for SEMIGROUP
		
		binary_opeators.add("*"); //multiplication
		binary_opeators.add(" times "); //multiplication, added to test multicharacter operators
	...
	private static Object applyUnaryOperator(Object O, String operator, VarBucket bucket) throws TreeNodeException
	{
			Object ans=null;
			// "!" or "inverse "
			if (operator.equals(unary_opeators.get(0))||operator.equals(unary_opeators.get(1)))
			{
			...
			else if (operator.equals(unary_opeators.get(2))) //$
			{
				if (O instanceof TreePair) {
			...
			else if (operator.equals(unary_opeators.get(3))) //@
			{
				if (O instanceof TreePair) {
					TreePair P = ((TreePair) O).duplicate();
					ans = P.reduce(false);
			...
			else if (operator.equals(unary_opeators.get(4)) || operator.equals(unary_opeators.get(5))) //growth and semigrowth 
			{
				if (O instanceof ArrayList) {
					...
					for (int i=0; i<L.size(); i++)
					{
						...
						if (i<L.size()-1)
						{
							...
							if (operator.equals(unary_opeators.get(4)))
							{
	...
	{
			TreePair ans=null;
			if (operator.equals(binary_opeators.get(0))||operator.equals(binary_opeators.get(1))) //"*", " times "
			{
			...
			else if (operator.equals(binary_opeators.get(2))) //"^" - conjugation and exponentiation
			{
				ans = null;
			...
			else if (operator.equals(binary_opeators.get(3))) //"#" commutator
			{
			...
			else if (operator.equals(binary_opeators.get(3))||operator.equals(binary_opeators.get(4))) //refine left pair
			{				
				if ((L instanceof TreePair)&&(L instanceof TreePair))
				{
					TreePair A = (TreePair)L;
					TreePair B = (TreePair)R;					
					if (operator.equals(binary_opeators.get(4))) //">"
					{
