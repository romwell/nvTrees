...
public class ExpressionParser {
	 ...
	 */
	private static ArrayList<String> unary_operators = new ArrayList<String>();
	 ...
	 * variable name).
	 */
	static String[] constant_formats = {"\\{(\\w|\\s|\\,)+\\}",	//TreePair format 
								"-?\\d+", //integer
								"\\[(\\w(?:,?\\s*))+\\]" //List format								
								};
	...
	private static void init()
	{
		 ...
		 * powers like A^-5
		 * 
		 * Unary operators must be applied before the variable, e.g. !A
		 * Multi-character operators are supported, but they must include leading/trailing spaces, e.g. " times "
		 * 
		 * NOTE: this code must be update if applyUnaryOperator / applyBinaryOperator is changed, and vice versa.
		 * TODO: write a saner system.
		 * 
		 * NOTE: the order in which the operators are added is the order in which they are processed.
		 * So don't move * above ^, for example.
		 */
		unary_operators.add("!"); //inverse
		unary_operators.add("$"); //reduce
		unary_operators.add("@"); //(un)reduce to smallest grid on the left
		unary_operators.add("growth "); //growth [g_1,g_2,..g_k,N] computes the sizes of B_1(id), ... B_N(id) in the GROUP generated by g1_,...,g_k 
		unary_operators.add("semi "); //same as growth, but for SEMIGROUP
	 ...
	 */
	public static void parse(String expression, VarBucket bucket) throws TreeNodeException
	{
		...
		try
		{
			...
			if (expression.contains("=")) //if the expression is an assignment, i.e. of the form a = ...
			{
				...
				if (!VarBucket.isGoodVarName(exp[0])) {throw new TreeNodeException("Invalid variable name: "+exp[0]);}
				storeVar = exp[0];
				expression = exp[1];
			}
			String A = parseExpression(expression, bucket);
			Object P = getAtomValue(A, bucket);
			if (storeVar!=null){bucket.add(storeVar, P);}
			bucket.add("ans", P); //always store the result of last computation in the "ans" variable!					
		}
	 ...
	 */
	private static String parseExpression(String expression, VarBucket bucket) throws TreeNodeException
	{
		expression = expression.trim();
	 ...
	 * are generated by nextVarName() function. This is used instead of building an expression tree.
	 * TODO replace storage of intermidiate results in the bucket with an expession tree.
	 * @throws an exception is thrown if the string contains an invalid expression
	 */
	private static String parseNoParenExpression(String expression, VarBucket bucket) throws TreeNodeException
	{
		...
		while(iU>-1)
		{
			...
			Object newO = applyUnaryOperator(O, operator, bucket);
		...
		while(iB>-1)
		{
			...
			Object newO = applyBinaryOperator(L, R, operator);
			String newVar = nextVarName();
			bucket.add(newVar, newO);
			auxVars.add(newVar);
	...
	/**
	 * TODO: write a saner system where an operator is added together with the funciton that processes it, instead of relying on indices
	 * Applies an unary operator to an object
	 * @param O the argument of the operator  
	 * @param operator the operator to be applied
	 * @return the result of the operation
	 */
	private static Object applyUnaryOperator(Object O, String operator, VarBucket bucket) throws TreeNodeException
	{
			Object ans=null;
			if (operator.equals("!"))
			{
			...
			else if (operator.equals("growth ") || operator.equals("semi ")) //growth and semigrowth 
			{
				if (O instanceof ArrayList) {
					...
					if (L.size() < 2)
					{
						...
						//throw new TreeNodeException("The operator " + unary_opeators.get(4) + " mut be applied to a list of the form [g1, .. g_k, N], where g_i are variables holding generators.");
					}
					ArrayList<TreePair> generators = new ArrayList<TreePair>();
					int N = 0;
					for (int i=0; i<L.size(); i++)
					{
						...
						if (i<L.size()-1)
						{
							Object T = getAtomValue((String) str, bucket);
							if (!(T instanceof TreePair))
							{
								throw new TreeNodeException("The variable " + str + " does not hold a TreePair");
							}
							generators.add((TreePair) T);
							if (operator.equals("growth "))
							{
								TreePair Tinv = TreePair.inverseOf(((TreePair) T));
								generators.add(Tinv);
							}
						}
						else
						{
							try
							{
								N = Integer.valueOf((String) str);
							}
							catch(NumberFormatException E)
							{
								throw new TreeNodeException(str +" is not a valid number");								
							}
						}
					}
					String growth_str="";
					int[] growth_values = TreePair.growth(generators, N);
					growth_str += growth_values[0];
					for (int i=1; i<growth_values.length; i++)
					{
						growth_str = growth_str + ", " + growth_values[i];
					}
					growth_str += "\n  \n";
					ans = growth_str;
					return ans;
	 ...
	 * @return the result of the operation
	 */
	private static Object applyBinaryOperator(Object L, Object R, String operator) throws TreeNodeException
	{
			TreePair ans=null;
			if (operator.equals("*")) //multiplication
			{
				if ((L instanceof TreePair)&&(R instanceof TreePair))
				{
					ans = TreePair.compose((TreePair)R, (TreePair)L);
					return ans;
				}
				else {throwOpException(operator, L, R);}
			}
			else if (operator.equals("^")) //"^" - conjugation and exponentiation
			{
				ans = null;
			...
			else if (operator.equals("#")) //"#" commutator
			{
			...
			else if (operator.equals("<")||operator.equals(">")) //refine left pair
			{				
				if ((L instanceof TreePair)&&(L instanceof TreePair))
				{
					TreePair A = (TreePair)L;
	...
	private static boolean holdsConstant(String S)
	{
		...
		{
			//show(S+" matches " + format + "? --> " + S.matches(format));
	...
	{
		if(S.matches(constant_formats[0]))
		{
			String T = S.substring(1, S.length()-1);
			return new TreePair(T);
		}
		else if(S.matches(constant_formats[1]))
		{
			Integer I = Integer.valueOf(S);
			return I;
		}
		else if (S.matches(constant_formats[2]))
		{
			String trimS = S.substring(1, S.length()-1);
			String[] splitS = trimS.split(",");
			ArrayList<String> L = new ArrayList<String>(Arrays.asList(splitS));
			return L;
		}
		else
		{
	 ...
	 */
	private static Object getAtomValue(String S, VarBucket bucket) throws TreeNodeException
	{		
		if (holdsConstant(S))
		{		
			//show("const: "+S);
